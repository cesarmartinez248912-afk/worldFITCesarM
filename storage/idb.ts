...
export async function loadOrSeedState(): Promise<AppState> {
  ...
  if (existing) {
    if (!("weekSchedules" in existing)) {
      const patched: AppState = { ...(existing as AppState), weekSchedules: [] };
      await saveState(patched);
      return patched;
    }
    if (existing.schemaVersion === CURRENT_SCHEMA_VERSION) return existing;
    await clearState();
  }
